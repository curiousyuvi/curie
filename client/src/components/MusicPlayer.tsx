...
const MusicPlayer = () => {
  const { currentTrack, paused, progress, active } = useRoomMusic();
  const { token } = useAuth();
    ...
    <div className="w-full absolute bottom-0 left-0 right-0 px-4 pb-4">
      <div className="w-full bg-indigo-500 flex flex-col rounded-lg px-2 drop-shadow-[0_35px_35px_rgba(0,0,0,0.8)]">
        {active ? (
          ...
          <div className="w-full h-20 flex justify-center items-center text-center text-gray-100">
            <p>Player not active, activate from Spotify Connect</p>
          </div>
        )}
      </div>
    </div>
