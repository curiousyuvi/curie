  ...
  reducers: {
    addRoom: (state, action: PayloadAction<RoomShort>) => {
      const exists = state.rooms.find(
        (room) => room.rid === action.payload.rid
      );
      if (exists) {
        const foundIdx = state.rooms.findIndex(
          (room) => room.rid == action.payload.rid
        );
        state.rooms[foundIdx] = { ...state.rooms[foundIdx], ...action.payload };
      } else state.rooms.push({ ...action.payload, messages: [] });
    },
    addMessage: (
      state,
      action: PayloadAction<{ rid: string; message: Message }>
    ) => {
      const exists = state.rooms.find(
        (room) => room.rid === action.payload.rid
      );
    ...
    addMessages: (
      state,
      action: PayloadAction<{ rid: string; messages: Message[] }>
    ) => {
      const exists = state.rooms.find(
        (room) => room.rid === action.payload.rid
      );
...
// export the action
export const { addMessage, addRoom, removeRoom, clearRooms, addMessages } =
  roomsSlice.actions;
